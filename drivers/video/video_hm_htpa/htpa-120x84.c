...
#define HM_HTPA_INIT_120X84(inst)                                                                  \
	static int16_t __aligned(4) hm_htpa_pixels_##inst[HTPA_FRAME_QUEUE_SIZE]                   \
				   [HTPA_WIDTH_120X84 * HTPA_HEIGHT_120X84];                     \
	static uint8_t __aligned(4)                                                               \
		hm_htpa_raw_top_##inst[HTPA_BLOCK_COUNT_120X84][HTPA_BLOCK_LENGTH_120X84];         \
	static uint8_t                                                                             \
		__aligned(4) hm_htpa_raw_bottom_##inst[HTPA_BLOCK_COUNT_120X84]                    \
						 [HTPA_BLOCK_LENGTH_120X84];                     \
	static uint8_t                                                                             \
		__aligned(4) hm_htpa_el_top_##inst[HTPA_BLOCK_LENGTH_120X84]; \
	static uint8_t                                                                             \
		__aligned(4) hm_htpa_el_bottom_##inst[HTPA_BLOCK_LENGTH_120X84]; \
