...
BUILD_ASSERT((160 * 120) <= UINT16_MAX, "Histogram counter type is too small");
#endif
...
#define HM_HTPA_INIT(inst)                                                                         \
	static int16_t __aligned(4) hm_htpa_pixels_##inst[HTPA_FRAME_QUEUE_SIZE][160 * 120];       \
	static uint8_t __aligned(4) hm_htpa_raw_top_##inst[12][1606];                              \
	static uint8_t                                                                             \
		__aligned(4) hm_htpa_raw_bottom_##inst[12][1606];                                 \
	static uint8_t __aligned(4) hm_htpa_el_top_##inst[1606];                                   \
	static uint8_t __aligned(4) hm_htpa_el_bottom_##inst[1606];                                \
